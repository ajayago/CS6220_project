...
from utils import *
from attention_modules import Attention_DK_Multi
...
class Generator(nn.Module):
    ...
    def __init__(self, latent_dim: int,
        ...
        """Parameters:
            ...
            config (dict): model architecture configuration as dictionary. Default None.
        """
        ...
        # Attention layer
        self.attention_module = Attention_DK_Multi(config) 
        
        # Init pretraining with or without attention
        self.pretrain_without_attention = config['pretrain_without_attention'] 
        
        if self.pretrain_without_attention:   
            self.add_attention = False
            print(f"Pretraining model without attention while gradient penalty is above {config['attention_threshold']}.")
        elif not self.pretrain_without_attention:
            self.add_attention = True

    def forward(self, x: torch.tensor, goi_dict_ppi:dict=None, goi_dict_corr:dict=None):
        ...
        Parameters:
            ...
            goi_dict_ppi (dict): dictionary with genes of interest for PPI.
            goi_dict_corr (dict): dictionary with genes of interest for CoExpression interactions.
        Returns:
        ...
        x = x.float()
        x = self.activation_func(self.proj1(x))  # Linear + activation function
        x = self.activation_func(self.proj2(x))
        x = self.activation_func(self.proj3(x))
        # Output (unconstrained)
        x = self.proj_output(x)

        # Attention based on dimension reduction with domain knowledge  
        if self.add_attention:
            # Residual connexion
            x = x + self.attention_module(x.clone(), goi_dict_ppi, goi_dict_corr)
...
class WGAN_GP_SA(object):
    """
    Conditional Wasserstein GAN with Gradient Penalty (WGAN-GP) + self-attention modules.
    """

    def __init__(self, config: dict):
        """ Parameters:
            config (dict): model architecture configuration as dictionary.
        """
        # Set architecture
        ...
        self.goi_dict_ppi = config['goi_dict_ppi']
        self.goi_dict_corr = config['goi_dict_corr']
    ...
    def gradient_penalty(
        ...
        Parameters:
            ...
            numerical_vars (tuple): tuple of conditional numerical variables
        Returns:
        ...
        # Generator forward pass with concatenated numerical and categorical
        # variables
        disc_inputs = interpolation # torch.cat((interpolation, numerical_vars), 1)

        # Get outputs from critic
        disc_outputs = self.D(disc_inputs, self.goi_dict_ppi, self.goi_dict_corr)# self.D(disc_inputs, categorical_vars, self.goi_dict_ppi, self.goi_dict_corr)
    ...
    def train_disc(
        ...
        Parameters:
            ...
            numerical_vars (torch.tensor): cpnditional numerical covariates.
        ...
        gen_inputs = z #torch.cat((z, numerical_vars), 1)
        gen_outputs = self.G(gen_inputs, self.goi_dict_ppi, self.goi_dict_corr)
        ...
        disc_outputs = self.D(gen_outputs, self.goi_dict_ppi, self.goi_dict_corr)
        disc_real = self.D(x , self.goi_dict_ppi, self.goi_dict_corr)
    ...
    def train_gen(
        ...
        gen_outputs = self.G( z, self.goi_dict_ppi, self.goi_dict_corr)
        NB_GENES = gen_outputs.shape[1]
    ...
    def train(self, TrainDataLoader,
        ...
        self.init_train(log_dir,
                        ...
                        is_gamma_fixed=config['is_gamma_fixed'],
                        pretrain_without_attention =config['pretrain_without_attention'])
        ...
        all_epochs_gamma_param = []
        all_epochs_gamma_param_disc = []
        ...
        for epoch in range(1, epochs + 1):
            ...
            for batch, _ in TrainDataLoader:
                ...
                # Numerical and categorical covariates to device
                ...
                # # no conditioning on tissue type
                # if self.categorical == 'no_tissue_type':
                #     _ = []
                #     encoded_tissue_types = []
                #     batch_categorical = []
                #     labels = labels.to(self.device)
                #     batch_numerical = torch.cat(
                #         (age.reshape(
            ...
            # Store gamma param evolution in Generator
            all_epochs_gamma_param.append(self.G.attention_module.gamma.detach().item())
            # Store if attention in Discriminator
            if config['attention_disc']:
                all_epochs_gamma_param_disc.append(self.D.attention_module.gamma.detach().item())
        ...
        #                         'gamma_param': self.G.attention_module.gamma.detach().item(),
        #                       'gamma_param_disc': self.G.attention_module.gamma.detach().item() if config['attention_disc'] else 0.,
        #                       'gen_loss': epoch_gen_loss,
        #                       'disc_loss': epoch_disc_loss,
        #                       'val_score': all_epochs_val_score[-1],
        #                       'fid_bin': all_epochs_fid_score[-1][0],
        #                       'fid_tissue': all_epochs_fid_score[-1][1],
        ...
        save_history(all_epochs_gamma_param, self.history_gamma_param_path)
        if config['attention_disc']:
            save_history(all_epochs_gamma_param_disc, self.history_gamma_param_disc_path)
    ...
    def init_train(
            ...
            attention_threshold:float=-1,
            is_gamma_fixed:bool=False, 
            pretrain_without_attention:bool=False):
        """
        ...
        Parameters:
            ...
            nb_principal_components (int): dimension of principal components reduction for analysis. Default 2000.
            attention_threshold (float): gradient penalty threshold to start training attention modules when `pretrain_without_attention` is True. Default -1.
            is_gamma_fixed (bool): whether attention gamma parameter is fixed or a learnable parameter. Default False.
            pretrain_without_attention (bool): whether to pretrain without attention. Default False.
        """
        # Optimizers
        ...
        self.history_prec_recall_train_PC_path = self.log_dir + '/precision_recall_train_pc.npy'
        ...
        self.history_gamma_param_path = self.log_dir+'/gamma_param.npy'
        self.history_gamma_param_disc_path = self.log_dir+'/gamma_param_disc.npy'
        ...
        self.UMAP_Printer = VisuUMAPEpoch(
            path=self.fig_dir, on_projections=True)
    ...
    def real_fake_data(
        ...
        with torch.no_grad():
            for batch, _ in ValDataLoader:
                ...
                # elif categorical == 'no_tissue_type':
                #     _ = []
                #     labels = labels.to(self.device)
                #     batch_categorical = []
                #     batch_numerical = torch.cat(
