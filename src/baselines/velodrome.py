...
# Here, 385 models are created, one for each drug and aggregated to get the results.
...
class arguments():
    def __init__(self):
        ...
        self.epoch = 30
        self.gpu = 0
        self.save_logs = f"./model_checkpoints/velodrome_logs_{sample_id}/"
        self.save_models = f"./model_checkpoints/velodrome_models_{sample_id}/"
        self.save_results = f"./model_checkpoints/velodrome_results_{sample_id}/"
...
class TrainVelodrome():
    def __init__(self, args=args):
        ...
        cl_train_df.index.name = "depmap_id"
        cl_responses_df = pd.read_csv("/data/ajayago/copied_from_cdal1/yiming_data_folder/dataset/CellLine/patient_auc.csv")
        
        unique_drugs = [
	    "CISPLATIN","PACLITAXEL","5-FLUOROURACIL","DOCETAXEL","GEMCITABINE","DOXORUBICIN", "CYCLOPHOSPHAMIDE"
        ]
        ...
        cl_test_df.index.name = "depmap_id"
        ...
        for drug_name in unique_drugs:
            ...
            train_drug_specific_df = cl_responses_df[cl_responses_df["drug_name"] == drug_name]# has depmap_id, drug_name, auc 
            train_mut_df = cl_train_df[cl_train_df.index.isin(train_drug_specific_df.depmap_id)]
            ...
            # filter out for the specific drug
            test_drug_specific_df = cl_responses_df[cl_responses_df["drug_name"] == drug_name]# has depmap_id, drug_name, auc 
            test_mut_df = cl_test_df[cl_test_df.index.isin(test_drug_specific_df.depmap_id)]
