...
class WGAN(object):
    ...
    def gradient_penalty(
        ...
        # variables
        # disc_inputs = torch.cat((interpolation, numerical_vars), 1)
        disc_inputs = interpolation

        # Get outputs from critic
        # disc_outputs = self.D(disc_inputs, categorical_vars)
        disc_outputs = self.D(disc_inputs)
    ...
    def train_disc(
        ...
        gen_outputs = self.G(gen_inputs)#self.G(gen_inputs, categorical_vars)
        ...
        # disc_outputs = self.D(torch.cat((gen_outputs, numerical_vars), 1), categorical_vars)
        # disc_real = self.D(torch.cat((x, numerical_vars), 1), categorical_vars)
    ...
    def train_gen(
        ...
        # gen_outputs = self.G( torch.cat((z, numerical_vars), 1), categorical_vars)
        gen_outputs = self.G(z)
        NB_GENES = gen_outputs.shape[1]
